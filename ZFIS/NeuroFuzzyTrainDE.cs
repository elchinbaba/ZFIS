        ...
        override public double EvaluateError(double[] parameters)
        {
            ...
            for(int p=0; p<NF.NLParams; p++){
                ...
                for(int i=0; i<NF.LParams[p].NTerms-1; i++){
                    ...
                    error+= System.Math.Max(0, NF.LParams[p].Terms[i+1].Value.AL-NF.LParams[p].Terms[i].Value.AR);
                }
                //
                for(int i=0; i<NF.LParams[p].NTerms; i++){
                    error+= System.Math.Max(0, LRanges[p,0]-NF.LParams[p].Terms[i].Value.AL);
                    error+= System.Math.Max(0, NF.LParams[p].Terms[i].Value.AR-LRanges[p,1]);
                }
            }
            ...
            for(int p=0; p<NF.NRParams; p++){
                ...
                for(int i=0; i<NF.RParams[p].NTerms-1; i++){
                    ...
                    //error+= System.Math.Max(0, NF.RParams[p].Terms[i+1].Value.AL-NF.RParams[p].Terms[i].Value.AR);
                }
                //
                for(int i=0; i<NF.RParams[p].NTerms; i++){
                    error+= System.Math.Max(0, RRanges[p,0]-NF.RParams[p].Terms[i].Value.AL);
                    error+= System.Math.Max(0, NF.RParams[p].Terms[i].Value.AR-RRanges[p,1]);
                }
            }
            return error;
            //
        ...
        */
        ...
        override public double EvaluateError(double[] parameters)
        {
            ...
            for(int p=0; p<NF.NLParams; p++){
                for(int i=0; i<NF.LParams[p].NTerms; i++){
                    err+= 
                        System.Math.Max(0, LRanges[p,0]-NF.LParams[p].Terms[i].Value.AM)+
                        System.Math.Max(0, NF.LParams[p].Terms[i].Value.AM-LRanges[p,1]);
                }
                for(int i=0; i<NF.LParams[p].NTerms-1; i++){
                    err+= System.Math.Max(0, NF.LParams[p].Terms[i].Value.AM-NF.LParams[p].Terms[i+1].Value.AM);
                }
            }
            if(!AdaptiveRT) return error;
            for(int p=0; p<NF.NRParams; p++){
                for(int i=0; i<NF.RParams[p].NTerms; i++){
                    err+= 
                        System.Math.Max(0, RRanges[p,0]-NF.RParams[p].Terms[i].Value.AM)+
                        System.Math.Max(0, NF.RParams[p].Terms[i].Value.AM-RRanges[p,1]);
                }
                for(int i=0; i<NF.RParams[p].NTerms-1; i++){
                    err+= System.Math.Max(0, NF.RParams[p].Terms[i].Value.AM-NF.RParams[p].Terms[i+1].Value.AM);
                }
            }
            return err;
        }
        */
        ...
        public int Save(string resfilename)
        {
            ...
            for(p=0; p<NF.NLParams; p++){
                f.WriteLine(NF.LParams[p].Label);
            ...
            for(p=0; p<NF.NRParams; p++){
                f.WriteLine(NF.RParams[p].Label);
            ...
            //
            f.WriteLine();
            f.WriteLine("Max Error: "+NF.MaxError(Inputs, Outputs));
            f.WriteLine("MSE: "+NF.MSE(Inputs, Outputs));
            f.WriteLine("RMSE: "+NF.RMSE(Inputs, Outputs));
            f.Close();
            //
            return 1; 
        }
