namespace ZFIS
{
    public class _NeuroFuzzy
    {
            ...
            void Inference2(double[] input)
            {
                ...
                for (r = 0; r < NF.NRules; r++)
                {
                    ...
                    for (p = 0; p < NF.NRParams; p++)
                    {
                        ...
                        // Not correct with this version of FuzzySet class:
                        //foutputL[p]|= s & rule_firing_L;
                        //foutputR[p]|= s & rule_firing_R;
                        // Use these instead:
        ...
        public void DoTrain(double[][] inputs, double[][] desoutputs, double[,] lranges, double[,] rranges, int iterations, int savefreq, int strategy_no)
        {
            _NeuroFuzzyTrainDE det = new _NeuroFuzzyTrainDE(this, inputs, desoutputs, lranges, rranges);
            DE.Optimizer deo = new DE.Optimizer(det);
            deo.SaveFrequency = savefreq; // log autosave frequency
            //deo.AutobreakEnabled= false; // new 17:27 31/10/2018
